...
- **Chatbot 页面检测**：自动识别 ChatGPT、Claude、Gemini、DeepSeek、Kimi、Agnes 等 AI 对话页面，生成基于对话轮次的目录结构
...
- **手动输入选择器**：站点配置对话框中可手工输入 CSS 或 XPath 选择器
- **配置保存**：将选择器保存为站点专属配置
- **自动排除**：自动排除扩展自身的UI元素
...
- **平滑滚动**：点击目录项平滑滚动到对应位置
- **空页面感知**：无目录内容且未配置自定义选择器的页面上工具条自动隐藏，出现标题后自动恢复
...
- **配置管理**：可查看、添加（CSS/XPath）、清空站点配置
- **多选择器支持**：同一站点可配置多个CSS/XPath选择器

### 🌐 多站点控制
- **按站点手动开启**：目录默认不在任何站点运行，点击工具栏图标可为当前站点开启/关闭
- **选项页**：全局审查与撤销各站点的启用状态和选择器配置（扩展详情 → 扩展选项）
...
6. 安装完成后访问任意网页，点击工具栏中的"网页目录助手"图标即可为该站点开启目录

### 基本操作

#### 1. 启用/禁用扩展

目录采用**按站点手动开启**：仅在点击图标启用过的站点上运行，未启用的页面不受任何打扰。点击浏览器工具栏中的"网页目录助手"图标可为当前站点开启目录，再次点击则关闭。

**效果**：
- 禁用状态（默认）：透明背景的白色文档图标为灰色，页面不注入任何内容
- 启用状态：点击图标后变为黑色，页面边缘出现吸附式目录工具条（按站点记忆偏好）
...
- **站点权限**：`http://*/*`、`https://*/*`（必需——安装时即授予，可读取你启用的任意页面；无需按站点单独授权）
...
- **防抖重建**：MutationObserver + 固定防抖（常规 400ms，流式内容 1200ms）避免频繁更新
...
- `tocSiteEnabledMap`：按站点手动开启记录——仅显式 `true` 表示启用，无记录即禁用
...
- **精简 UI** — ~~移除经典浮动面板模式，仅保留 Edge Dock 作为唯一 UI 模式~~ *(已完成 — v1.6.0 移除 Classic UI)*
...
- **开箱即用** — 安装即用，零配置
- **智能启用判断** — ~~引入页面类型判断，仅对以文字内容为主的页面自动启用目录，避免在不合适的页面上打扰用户~~ *（已在 v1.12 以另一种方式解决——扩展改为按站点手动开启：目录仅出现在显式启用的站点上，非阅读类页面不会再被打扰。）*
