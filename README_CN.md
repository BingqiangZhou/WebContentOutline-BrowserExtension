...
目录默认**在所有网站上启用**——任意包含标题的页面都会自动出现吸附式目录工具条，无需设置。点击浏览器工具栏中的"网页目录助手"图标可切换当前站点的开关。

**效果**：
- 启用状态（默认）：透明背景的白色文档图标为黑色，页面边缘出现吸附式目录工具条
- 禁用状态：点击图标后变为灰色并移除该站点的工具条（按站点记忆偏好）
...
- **开箱即用** — 安装即用，零配置
- **智能启用判断** — 当前默认对所有网页启用目录（可逐站手动关闭），因此许多非阅读类页面（如后台管理、图库、视频站）也会出现目录，只能手动关闭或忽略。计划引入页面类型判断，仅对以文字内容为主的页面自动启用目录，避免在不合适的页面上打扰用户。该能力预计随 v2.0 的 AI 功能一同引入。
