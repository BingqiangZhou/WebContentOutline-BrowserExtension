...
- **✅ 自动内容区域检测**：自动识别页面的主要内容区域，过滤导航栏、侧边栏和页脚
- **✅ Chatbot 页面检测**：自动识别 ChatGPT、Claude、Gemini、DeepSeek、Kimi 等 AI 对话页面，生成基于对话轮次的目录
- **自适应识别**：后续版本将持续优化，学习常见页面布局和框架的规律，提升识别准确率

手动 CSS/XPath 选择器配置仍作为高级选项保留，用于边缘场景。
