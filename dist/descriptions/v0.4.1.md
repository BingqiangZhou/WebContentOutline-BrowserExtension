...
- 🐛 **Icon Toggle Fix** - Fixed extension icon not switching between enabled/disabled states when clicked
- 🔧 **API Modernization** - Updated chrome.action and chrome.storage APIs to use Promise-style for Manifest V3 compatibility
- 🛡️ **Error Handling Improvements** - Enhanced extension context invalidation handling with silent failure mode
...
- 🐛 **图标切换修复** - 修复扩展图标点击时无法在启用/禁用状态间切换的问题
- 🔧 **API 现代化** - 更新 chrome.action 和 chrome.storage API 使用 Promise 风格以适配 Manifest V3
- 🛡️ **错误处理改进** - 增强扩展上下文失效处理，静默失败模式避免干扰用户体验
